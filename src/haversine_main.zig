...
pub fn handleCompute(allocator: std.mem.Allocator, io: std.Io, input_file_path: []const u8, compare_to_path: ?[]const u8) !void {
    ...
    // memory mapping the file would be faster
    ...
    const node = json.node orelse return error.JsonNodeNull;
    ...
    if (compare_to_path) |*path| {
        ...
        const expected_data_buf = try allocator.alignedAlloc(u8, .@"64", compare_to_file_size);
